...
interface EditAnkeworkPageProps {
  params: Promise<{ id: string }>;
}

export default function EditAnkeworkPage({ params }: EditAnkeworkPageProps) {
  ...
  const fetchAnkework = async (workId: string) => {
    try {
      const response = await fetch(`/api/admin/ankeworks/${workId}`);
      ...
      setFormData({
        ...
        vote_budget: data.vote_budget,
        guest_check: data.guest_check,
        status: data.status
      });
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">アンケワークス投稿 編集</h1>
      </div>
